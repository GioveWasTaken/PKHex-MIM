...
public class PasteImporter : AutoModPlugin
{
    ...
    private void ImportPaste(object? sender, EventArgs e)
    {
        ...
        if (!AutoFixUntilLegal.TrySelectBox(sav, SaveFileEditor.CurrentBox, out var box))
            return;

        var result = MakeItMineBoxImporter.ImportTeamToBox(sav, sets, box);
        WinFormsUtil.Alert(result.Report);
        if (result.Success)
            SaveFileEditor.ReloadSlots();
    }

    private static List<ShowdownSet> GetSets(string source)
    {
        if (ShowdownUtil.IsTeamBackup(source))
            return ShowdownTeamSet.GetTeams(source).SelectMany(z => z.Team).ToList();

        return ShowdownUtil.ShowdownSets(source).ToList();
    }
